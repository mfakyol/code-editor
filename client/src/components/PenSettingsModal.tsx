...
import type { CssPreprocessor, HtmlPreprocessor, JsPreprocessor, SettingsTab } from '@/types/preprocessors'
...
function PenSettingsModal() {
  const { isOpen, activeTab, settings, closeSettings, updateSettings } = usePenSettings()
  const { t } = useI18n()
        ...
        <div className="flex min-h-0 flex-1">
          <nav className="w-44 shrink-0 border-r border-neutral-700 bg-neutral-950 py-2">
            {tabs.map((item) => (
              ...
              >
                {tab === item.id && <span className="absolute inset-y-1 left-0 w-1 rounded-r bg-emerald-500" />}
                {item.label}
              </button>
            ))}
          </nav>
          ...
          <div className="min-w-0 flex-1 overflow-auto p-6">
            ...
            {tab === 'scripts' && (
              <ResourceList
                ...
                onChange={(items) => updateSettings({ externalScripts: items })}
              />
            )}
            ...
            {tab === 'styles' && (
              <ResourceList
                ...
                onChange={(items) => updateSettings({ externalStyles: items })}
              />
            )}
          </div>
        </div>

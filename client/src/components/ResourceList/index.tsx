...
function ResourceList({
  ...
  const update = (index: number, url: string) =>
    onChange(items.map((item, i) => (i === index ? url : item)))
  const remove = (index: number) =>
    onChange(items.filter((_, i) => i !== index))

  // Debounced cdnjs search.
  ...
  return (
    <div>
      ...
      <div className="relative mb-3">
        ...
        />
        {searching && (
          <IconLoader2 className="absolute right-2.5 top-2.5 h-4 w-4 animate-spin text-neutral-500" />
        )}
      </div>
      ...
      {items.length === 0 && (
        <p className="mb-3 text-sm text-neutral-500">{emptyText}</p>
      )}
